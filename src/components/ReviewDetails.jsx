...
const ReviewDetails = () => {
    ...
    const watchItem = { email, name, genre, rating, description, gameTitle, gameCoverUrl };

    // Fetch watchlist when component loads
    // useEffect(() => {
    //     fetch(`http://localhost:3000/watchlist?email=${email}`)
    //         .then(res => res.json())
    ...
    const handleWatchList = () => {
        const isDuplicate = watchList.some(
            item => item.gameTitle === watchItem.gameTitle && item.email === watchItem.email 
        );
